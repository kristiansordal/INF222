...
eval''' (Plus e1 e2) = VVI (zipWith (+) e1' e2')
  where
    VVI e1' = eval''' e1
    VVI e2' = eval''' e2
eval''' (Mult e1 e2) = VVI [s1, s2, s3]
  where
    ...
    s2 = a3 * b1 - a1 * b3
    s3 = a1 * b2 - a2 * b1
eval''' (Minus e1 e2) = VVI (zipWith (-) e1' e2')
  where
...
eval''' (Not e1) = VVB (not e1')
  where
    VVB e1' = eval''' e1
eval''' (Le e1 e2) = VVB (all (uncurry (<=)) $ zip e1' e2')
  where
    VVI e1' = eval''' e1
    VVI e2' = eval''' e2
eval''' (Ifte e0 e1 e2) = if e0' then eval''' e1 else eval''' e2
...
test_evaluations = do
  ...
  let resInt = map (natToInt . eval) (exprList "Zero" "Succ Zero" "Succ (Succ Zero)" "Succ (Succ (Succ Zero))")
